...
- **日期**：2026-08-27
- **症状**：某个 Host 需要级联目录、组织解析、业务联动或特殊展示时，如果直接改扩展包默认行为，其他消费者即使没有该业务也会被迫适配，形成隐性破坏性升级和包间依赖。
- **根因**：没有区分“所有消费者共享的稳定机制”和“单个 Host 的业务组合”，把 Host 需求下沉成包内定性；或者让一个扩展包直接引用另一个业务包，导致依赖方向倒置。
- **解法**：扩展包只提供稳定默认、窄 Contract/Resolver 和必要的能力检测；Host 实现契约并在胶水层完成个性化查询、级联选项、事件/Observer 联动和响应适配。未实现扩展契约的 Host 必须保持原行为；公共契约调整前后分别验证默认 Host 与个性化 Host，不能只验证提出需求的消费者。发现需求可能改变包默认、影响其他 Host 或引入包间依赖时，必须先列出消费者与边界，沟通确认后再开工。
