...
- **destroy-forever 动作声明 `danger: 1`**：`@acl` 注解新增危险标记，表示该动作高危且不可逆。授权页的模块/控制器「全选」据此**排除**这些权限点，避免误勾 —— 必须由操作者逐个显式勾选。语义由注解显式声明，前端不再按文案猜测（文案受多语言与改名影响，而 ACL key 经 md5 后前端只有 `{id,pid,label}`，无从识别 `-force-destroy`）。仅注解与产物变化，无运行时行为变更。
## [0.1.7] - 2026-09-21
